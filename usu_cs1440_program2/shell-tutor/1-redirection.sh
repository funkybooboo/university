...
_damaged_files_check() {
	...
	if [[ $(git hash-object "$_BASE/first.md" 2>/dev/null) != "$FIRST_FILE_HASH" ||
		  $(git hash-object "$_BASE/second.md" 2>/dev/null) != "$SECOND_FILE_HASH" ||
		  $(git hash-object "$_BASE/third.md" 2>/dev/null) != "$THIRD_FILE_HASH" ||
		  $(git hash-object "$_BASE/nameError.py" 2>/dev/null) != "$NAME_ERROR_HASH" ]]; then
		cat <<-MSG
...
redir_stderr_nameerror_prologue() {
	cat <<-MSG
	Redirection of $(_STDERR) uses a slightly different redirection operator.  $(cmd '2>') redirects $(bld just) $(_STDERR standard error) to a file, leaving $(_STDOUT) to the terminal.
...
edit_outputstreams_py_test() {
	...
	if   ! grep -q -E "explicitly output to STDOUT.*, *file=sys.stdout" "$_BASE/outputStreams.py"; then
		return $_NO_STDOUT
	elif ! grep -q -E "also put output on STDERR.*, *file=sys.stderr" outputStreams.py; then
		return $_NO_STDERR
	fi
}
...
epilogue() {
	...
	* $(path /dev/null)
	...
	Run $(cmd ${SHELL##*/} make-certificate) to generate your certificate of completion.
